...
CQS(Command-Query Separation) 준수와 유사 패턴 중복 추출 여부도 함께 검토한다.
...
- **CQS 준수**: 상태 변경 메서드는 데이터를 반환하지 않고, 조회 메서드는 상태를 변경하지 않는지
- **유사 패턴 중복 추출**: 검증/분기/예외 처리/DTO 변환 패턴이 반복되면 공통 로직으로 추출되었는지
