...
**작성하지 않는 테스트 유형** (불필요한 테스트 방지):
- **내부 구현 테스트 금지**: private/protected 메서드(`_build_payload`, `_request`, `_parse_response` 등)의 내부 동작을 직접 테스트하지 않는다. 공개 인터페이스를 통해 간접적으로 검증한다.
- **메서드 위임 테스트 금지**: `put()`이 `_request('PUT', ...)`를 호출하는지 등 단순 위임 관계를 assert하지 않는다.
- **예외 클래스 속성 테스트 금지**: 예외의 생성자 기본값, 속성 접근, `__cause__` 체이닝 등 언어/프레임워크 기능이므로 테스트하지 않는다.
- **DB 스키마상 불가능한 시나리오 금지**: AUTO_INCREMENT PK가 0이나 음수인 경우, NOT NULL 컬럼에 NULL이 들어오는 경우 등 DB 제약조건이 이미 방지하는 시나리오는 테스트하지 않는다.

**테스트 작성 패턴**:
- **`pytest.mark.parametrize`로 반복 제거**: 동일 패턴의 테스트가 2개 이상이면 parametrize로 통합한다.
```python
# BAD — 동일 패턴 N개 반복
def test_reject_from_approve_raises(self): ...
def test_reject_from_reject_raises(self): ...

# GOOD — parametrize로 통합
@pytest.mark.parametrize("fixture_name", ["approved_entity", "rejected_entity"])
def test_reject_from_invalid_status_raises(self, fixture_name, request):
    entity = request.getfixturevalue(fixture_name)
    with pytest.raises(InvalidStateTransition):
        entity.reject(reviewer_id=1, reason='test')
```
- **Fake Repository 패턴**: `unittest.mock.patch` 대신 Protocol/ABC를 구현하는 in-memory dict 기반 Fake 클래스를 사용한다. Fake는 `tests/fake_object.py` 또는 `tests/fakes/` 디렉토리에 배치한다.
- **DTO로 타입 안전성**: API 호출 테스트에서 raw dict 대신 frozen dataclass DTO를 사용한다.
