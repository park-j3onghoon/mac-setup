...
- Claude Code: 기본 `effortLevel`은 `medium`(settings.json). 모델이 난이도를 자동 판단해 그 안에서 스케일한다. 특정 작업에 더 깊은 사고가 필요하면 그 요청에 **`ultrathink`**(또는 `think hard`)를 붙여 해당 턴만 최대로 올린다.
- Codex: `~/.codex/config.toml`의 `model_reasoning_effort = "xhigh"`는 리뷰·근본원인 진단 위임 품질을 위해 유지한다.
...
- **payments-api 레포 작업 시 `~/.claude/payments-api-rules.md`를 읽고 적용한다** (날짜 타입: 신규 코드 LocalDate 금지·date 사용 등).
...
- Plan·분석·change-summary 등 **작업 설계 문서는 repo 내부 `docs/`가 아닌 `~/plans/{repo이름}/{작업명}/` 하위**에 저장한다.
  - 예: `~/plans/ad-server/ads-168-create-view-connect/plan.md`
...
- **회사 repo**(example 등 팀/조직 repo): `/review` → `/pr-size-check`, `teddy/` 브랜치, PR 생성 규칙(assignee 등) 전부 유지.
...
- **PR 제목 = `<prefix>: [<Linear 카드 ID>] <설명>`** (예: `fix: [ISSUE-000] 정산 메일 중복 발송 방어`).
...
- 대신 **① 전용 검색 도구(Grep/Glob/Read) 우선, ② 불가피하게 셸을 쓰면 절대경로를 인자로 넘긴다.**
  - ❌ `cd ~/example/postback-api && grep -rn "foo" --include="*.py" .`
  - ✅ `grep -rn "foo" --include="*.py" /Users/teddy.park/example/postback-api`
