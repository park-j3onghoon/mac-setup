...
- **API 추가 시 OpenAPI YAML 스펙 문서도 함께 업데이트** (프로젝트가 collaborative/ad-server 등 스펙 문서 컨벤션을 따른다면).
...
- **모델 `updated_at`은 DDL/ORM 자동 처리**에 맡긴다. 수동 세팅 금지 (중복 처리 및 race 위험).
  - **example DB 컨벤션**: `updated_at` 컬럼은 DDL에 `ON UPDATE CURRENT_TIMESTAMP`가 걸려 있어 DB가 자동 갱신한다. 따라서 `queryset.update()`처럼 `auto_now=True` signal이 안 도는 경우에도 코드에서 `updated_at=datetime.now(...)`를 따로 넣지 않는다. (`save()`는 ORM의 `auto_now`로 처리됨.) 코드에서 수동 세팅하면 DB 트리거와 이중 처리되고, 다른 트랜잭션과 race 가능.
