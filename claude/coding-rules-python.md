...
- **OpenAPI spec with every endpoint change**: ad-server `docs/apis/paths/adscenter/{module}/user.yaml|admin.yaml` (pattern: `collaborative/`, `livecommerce/`); mobile-app one yaml per endpoint under `docs/openapi/{app}/` registered by `$ref` in `docs/openapi/root.yaml` `paths:`; add it whenever a similar endpoint is documented; file = top-level `post:`/`get:` + summary/tags/operationId/requestBody·parameters/responses 200/202/400/401/403/500.
