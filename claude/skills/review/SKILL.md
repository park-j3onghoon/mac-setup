...
역할: 팀 리뷰어(REVIEWER_1, REVIEWER_2, REVIEWER_3, REVIEWER_4, REVIEWER_5, REVIEWER_6, REVIEWER_10, REVIEWER_11, REVIEWER_12, REVIEWER_13, REVIEWER_14, REVIEWER_15)별 관점 + 언어별 리뷰어(REVIEWER_22, REVIEWER_11, REVIEWER_23, REVIEWER_24, REVIEWER_4)
...
PR이 CLAUDE.md 또는 AGENTS.md를 변경했다면 페어 파일이 같이 변경되었는지 확인한다. 페어 위치: `~/.claude/CLAUDE.md` ↔ `~/.codex/AGENTS.md`, `~/example/CLAUDE.md` ↔ `~/example/AGENTS.md`, `~/example_analysis/CLAUDE.md` ↔ `~/example_analysis/AGENTS.md`.
...
글로벌 또는 Example scope에서 한쪽 파일을 수정했다면 페어 파일도 같이 업데이트한다. 도구별 차이(assignee 등)만 분기 섹션으로 유지.
...
2. **언어/도메인 특화**:
   ...
   - Vue + Example 메인 프로젝트(admin-center, dash 등) → `~/.claude/coding-rules-vue.md`
