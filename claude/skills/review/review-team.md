...
```
### R{N} ({이름}): N건
- [CRITICAL/INFO] file:line — 설명 (카테고리: C{N})
```
해당 없으면: `### R{N} ({이름}): PASS`

리뷰 결과를 사용자에게 최종 보고할 때만 R{N} → 이름으로 변환.
카테고리 reference (C{N}) 는 본 문서를 참조하여 핵심 패턴 확인 가능.

각 리뷰어의 **전문 도메인** 은 매핑표 참조. 리뷰 시뮬레이션 시 해당 리뷰어가 강조하는 카테고리 (`— R{N}` reference) 위주로 점검.
